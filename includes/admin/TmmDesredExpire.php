...
//MyWpListTable{table_name} like TmmDesredExpire

if( ! class_exists( 'WP_List_Table' ) ) {
    require_once( ABSPATH . 'wp-admin/includes/class-wp-list-table.php' );
}

class TmmDesredExpire extends WP_List_Table{
    ...
    public function process_bulk_action() { 
        ...
        if ( 'trash' === $this->current_action() ) {
            ...
            if ( ! wp_verify_nonce( $nonce, 'sp_nonce_customer' ) ) {
               ...
               // $pp = admin_url().'admin.php?page=tmm-desred-expire';
                //wp_redirect($pp);
    ...
    private function table_data(){ 
        ...
        $sql = "SELECT * FROM {$wpdb->prefix}tmm_expire_tb";
    ...
    */
        ...
        if ($current == 'trash') {
            ...
            $actions = [
                ...
                'trash' => sprintf( '<a href="?page=%s&action=%s&customer=%s&_wpnonce=%s">Trash</a>', esc_attr( $_REQUEST['page'] ), 'trash', absint( $item['ID'] ), $sp_nonce_customer ),
                 'view' => '<a class="view_data" href="#" data-ID="'.$item['ID'].'" data-tbn="tmm_expire_tb" data-sourse-type="">View</a>',
            ];
    ...
    function extra_tablenav( $which ) {
        ...
        if ( $which == "top" ){   
            get_filter_by_colum_name($this->get_columns(), 'tmm_expire_tb');
        }
        if ( $which == "bottom" ){

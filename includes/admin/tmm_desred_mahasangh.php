...
//my_wp_list_table_{table_name} like  tmm_desred_mahasangh  tmm-desred-mahasangh 

/*add_action( 'admin_menu', 'tmm_desred_mahasangh_admin_menu_page_fun');
function tmm_desred_mahasangh_admin_menu_page_fun(){

    $title = "desred mahasangh";
    $hook = add_menu_page( $title, $title, 'manage_options', 'tmm-desred-mahasangh', 'tmm_desred_mahasangh_admin_menu_fun');

    add_action( "load-$hook", 'tmm_desred_mahasangh_add_option');

}*/

function tmm_desred_mahasangh_admin_menu_fun(){
?>  
<div class="wrap">
        <?php
            global $wpdb;
...
function tmm_desred_mahasangh_edit_form_fun($result){
   ...
   ?>
    <div class="form_field">
        <label><strong><?php _e( 'Lm number', 'tmm-desred-mahasangh-funds' ); ?></strong></label>
        <?php
    ...
    <div class="form_field">
        <label><strong><?php _e( 'Amount', 'tmm-desred-mahasangh-funds' ); ?></strong></label>
        <input type="text" name="amount" placeholder="amount" value="<?php echo $amount?>" class="large-text"> 
    </div>

    <div class="form_field">
        <label><strong><?php _e( 'Year', 'tmm-desred-mahasangh-funds' ); ?></strong></label>
    ...
    <input type="hidden" name="action" value="tmm_desred_mahasangh_Form_Action">
   <?php

}

function tmm_desred_mahasangh_submit_btn_fun(){
    ?>
...
function tmm_desred_mahasangh_add_form_fun(){
...
function tmm_desred_mahasangh_add_option() {

    add_meta_box("tmm-desred-mahasangh-edit-form", "Member", "tmm_desred_mahasangh_edit_form_fun", "tmm-desred-mahasangh-edit-form", "normal");

    add_meta_box("tmm-desred-mahasangh-add-form", "Member", "tmm_desred_mahasangh_add_form_fun", "tmm-desred-mahasangh-add-form", "normal");

    add_meta_box("tmm-desred-mahasangh-submit-btn", "Publish", "tmm_desred_mahasangh_submit_btn_fun", "tmm-desred-mahasangh-submit-btn", "side");
...
add_filter('set-screen-option', 'tmm_desred_mahasangh_set_option', 10, 3);
 
function tmm_desred_mahasangh_set_option($status, $option, $value) {
...
add_action('admin_post_tmm_desred_mahasangh_Form_Action', 'tmm_desred_mahasangh_Form_Action');
add_action('admin_post_nopriv_tmm_desred_mahasangh_Form_Action', 'tmm_desred_mahasangh_Form_Action');
function tmm_desred_mahasangh_Form_Action(){
    ...
    if ($_POST['mode'] == 'add') { 
        ...
        if($wpdb->last_error !== '') { 
            ...
            $redirect_url = admin_url().'admin.php?page=tmm-desred-mahasangh&action=add';
    ...
    if ($_POST['mode'] == 'edit') {
        ...
        if($wpdb->last_error !== '') { 
            ...
            $redirect_url = admin_url().'admin.php?page=tmm-desred-mahasangh&action=edit&customer='.$lastid;
            wp_redirect($redirect_url);
            die();
...
function tmm_desred_mahasangh_admin_notice__success() {

  $screen = get_current_screen();

  //if ( $screen->id !== 'toplevel_page_tmm-desred-mahasangh') return;

    if (isset($_SESSION["add_message_session"])) {
        ?>
        <div class="notice notice-success is-dismissible">
            <p><?php _e( $_SESSION['add_message_session'], 'tmm-desred-mahasangh-funds' ); ?></p>
        </div>
        <?php
    ...
    if (isset($_SESSION["edit_message_session"])) {
        ?>
        <div class="notice notice-success is-dismissible">
            <p><?php _e( $_SESSION['edit_message_session'], 'tmm-desred-mahasangh-funds' ); ?></p>
        </div>
        <?php
    ...
    if (isset($_SESSION["add_error_message_session"])) {
        ...
        <div class="notice notice-error is-dismissible">
            <p><?php _e( $_SESSION['add_error_message_session'], 'tmm-desred-mahasangh-funds' ); ?></p>
        </div>
        <?php
...
add_action( 'admin_notices', 'tmm_desred_mahasangh_admin_notice__success' );
...
//MyWpListTable{table_name} like TmmDesredExpenses

if( ! class_exists( 'WP_List_Table' ) ) {
    require_once( ABSPATH . 'wp-admin/includes/class-wp-list-table.php' );
}

class TmmDesredExpenses extends WP_List_Table{
    ...
    public function process_bulk_action() { 
        ...
        if ( 'trash' === $this->current_action() ) {
            ...
            if ( ! wp_verify_nonce( $nonce, 'sp_nonce_customer' ) ) {
                ...
                /*$pp = admin_url().'admin.php?page=tmm-desred-expenses';
                wp_redirect($pp);
                exit;*/
            }
    ...
    private function table_data(){ 
        ...
        $sql = "SELECT * FROM {$wpdb->prefix}tmm_expenses_tb";
    ...
    public function column_default( $item, $column_name ){
        ...
        switch( $column_name ) {
            ...
            $thumbnail_id = tmm_desred_get_image($item['ID'], 'expenses');
    ...
    */
    public function column_department( $item ) {
        ...
        //$title = '<strong>' . get_member_name_by_id($item['member_id']) . '</strong>';

        $title = '<strong>' . $item['department'] . '</strong>';
        ...
        if ($current == 'trash') {
            ...
            $actions = [
                ...
                'view' => '<a class="view_data" href="#" data-ID="'.$item['ID'].'" data-tbn="tmm_expenses_tb" data-sourse-type="expenses">View</a>',
            ];
    ...
    function extra_tablenav( $which ) {
        ...
        if ( $which == "top" ){   
            get_filter_by_colum_name($this->get_columns(), 'tmm_expenses_tb');
        }
        if ( $which == "bottom" ){

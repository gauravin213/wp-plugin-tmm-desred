...
//MyWpListTable{table_name} like TmmDesredAnshdan

if( ! class_exists( 'WP_List_Table' ) ) {
    require_once( ABSPATH . 'wp-admin/includes/class-wp-list-table.php' );
}

class TmmDesredAnshdan extends WP_List_Table{
    ...
    public function process_bulk_action() { 
        ...
        if ( 'trash' === $this->current_action() ) {
            ...
            if ( ! wp_verify_nonce( $nonce, 'sp_nonce_customer' ) ) {
                ...
                /*$pp = admin_url().'admin.php?page=tmm-desred-anshdan';
                wp_redirect($pp);
                exit;*/
            }
    ...
    private function table_data(){ 
        ...
        $sql = "SELECT * FROM {$wpdb->prefix}tmm_anshdan_tb";
    ...
    */
    public function column_lm_number( $item ) {
        ...
        if ($current == 'trash') {
            ...
            $actions = [
                ...
                'view' => '<a class="view_data" href="#" data-ID="'.$item['ID'].'" data-tbn="tmm_anshdan_tb" data-sourse-type="">View</a>',
            ];
    ...
    function extra_tablenav( $which ) {
        ...
        if ( $which == "top" ){   
        get_filter_by_colum_name($this->get_columns(), 'tmm_anshdan_tb');
        }
        if ( $which == "bottom" ){

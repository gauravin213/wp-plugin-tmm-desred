...
//MyWpListTable{table_name} like TmmDesredSangh

if( ! class_exists( 'WP_List_Table' ) ) {
    require_once( ABSPATH . 'wp-admin/includes/class-wp-list-table.php' );
}

class TmmDesredSangh extends WP_List_Table{
    ...
    public function process_bulk_action() { 
        ...
        if ( 'trash' === $this->current_action() ) {
            ...
            if ( ! wp_verify_nonce( $nonce, 'sp_nonce_customer' ) ) {
               ...
               // $pp = admin_url().'admin.php?page=tmm-desred-sangh';
                //wp_redirect($pp);
    ...
    private function table_data(){ 
        ...
        $sql = "SELECT * FROM {$wpdb->prefix}tmm_sangh_tb";
    ...
    */
    public function column_lm_number( $item ) {
        ...
        if ($current == 'trash') {
            ...
            $actions = [
                ...
                'view' => '<a class="view_data" href="#" data-ID="'.$item['ID'].'" data-tbn="tmm_sangh_tb" data-sourse-type="">View</a>'
            ];
            ...
            $actions = [
                'edit' => sprintf( '<a href="?page=%s&action=%s&customer=%s&_wpnonce=%s">Edit</a>', esc_attr( $_REQUEST['page'] ), 'edit', absint( $item['ID'] ), $sp_nonce_customer ),
                'trash' => sprintf( '<a href="?page=%s&action=%s&customer=%s&_wpnonce=%s">Trash</a>', esc_attr( $_REQUEST['page'] ), 'trash', absint( $item['ID'] ), $sp_nonce_customer ),
                'staus_approve' => sprintf( '<a href="?page=%s&action=%s&customer=%s&_wpnonce=%s">Approve</a>', esc_attr( $_REQUEST['page'] ), 'staus_approve', absint( $item['ID'] ), $sp_nonce_customer )
            ];
    ...
    protected function get_views() { 
      $views = array();
       ...
       //Trash link
       /*$pending = add_query_arg('customer_status','pending');
       $class = ($current == 'pending' ? ' class="current"' :'');
       $views['pending'] = "<a href='{$pending}' {$class} >Pending</a>";*/
    ...
    function extra_tablenav( $which ) {
        ...
        if ( $which == "top" ){   
            get_filter_by_colum_name($this->get_columns(), 'tmm_sangh_tb');
        }
        if ( $which == "bottom" ){
